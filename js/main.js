...
function buildFooterSocial(){
  document.querySelectorAll('footer .wrap').forEach(w=>{
    ...
    if(!w.querySelector('.foot-tagline') && (C.footer_tagline_en||C.footer_tagline_ar)){
      const tg=el('p','foot-tagline',L(C.footer_tagline_en,C.footer_tagline_ar));
      const brand=w.querySelector('.foot-brand');
      if(brand) brand.after(tg); else w.prepend(tg);
    }
  });
...
function bindCursor(){
  ...
  document.querySelectorAll('a,button,.case,.chip,.cc,.hcard,.brand-chip').forEach(n=>{
    n.addEventListener('mouseenter',()=>ring.classList.add('hover'));
    n.addEventListener('mouseleave',()=>ring.classList.remove('hover'));
  });
}
/* ---------------- MAGNETIC ---------------- */
...
function initCardFx(){
  ...
  let cur=null;
  document.addEventListener('mousemove',e=>{
    const card=(e.target.closest&&e.target.closest('.hcard, .case, .cpag, .cc, .case-file'))||null;
...
function initBgField(){
    ...
    for(let i=0;i<parts.length;i++){ const p=parts[i], gx=(p.x/CELL)|0, gy=(p.y/CELL)|0;
      for(let ox=-1;ox<=1;ox++)for(let oy=-1;oy<=1;oy++){ const cell=grid.get((gx+ox)+'|'+(gy+oy)); if(!cell)continue;
        for(let n=0;n<cell.length;n++){ const j=cell[n]; if(j<=i)continue; const q=parts[j];
          ...
          if(d2<R2D){ const d=Math.sqrt(d2), a=1-d/D; let al=0.15*a;
            if(hasMouse){ const mdx=mx-(p.x+q.x)*0.5, mdy=my-(p.y+q.y)*0.5, md=mdx*mdx+mdy*mdy; if(md<52900) al+=0.30*a*(1-Math.sqrt(md)/230); }
  ...
  function drawParticles(){
    for(const p of parts){
      ...
      if(near>0){ ctx.fillStyle='rgba(120,180,235,'+(0.19+0.55*near).toFixed(3)+')'; ctx.beginPath(); ctx.arc(p.x,p.y,p.r*(1+near*0.9),0,6.283); ctx.fill(); }
  ...
  function loop(now){
    ...
    if(hasMouse){
      ...
      const Rg=210, g=ctx.createRadialGradient(mx,my,0,mx,my,Rg);
      g.addColorStop(0,'rgba(150,196,236,0.24)');
      g.addColorStop(0.30,'rgba('+ACC+',0.13)');
      g.addColorStop(1,'rgba(95,158,219,0)');
      ctx.fillStyle=g; ctx.beginPath(); ctx.arc(mx,my,Rg,0,6.283); ctx.fill();
